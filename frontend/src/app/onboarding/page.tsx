...
export default function OnboardingPage() {
  ...
  const { mutate: createPack, isPending: creatingPack } = useMutation({
    mutationFn: () => packsApi.create({ name: packName, description: packDesc || undefined, goalType }),
    onSuccess: () => setStep(3),
  });

  const { mutate: joinPack, isPending: joiningPack } = useMutation({
    mutationFn: () => packsApi.join(inviteCode),
    onSuccess: () => setStep(3),
  });
              ...
              <div className="flex flex-col items-center gap-1">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
                  ...
                  {step > n
                    ...
                    : <Icon className={`w-4 h-4 ${step >= n ? 'text-white' : 'text-gray-500'}`} />
                  }
                </div>
                <span className={`text-[10px] font-medium ${step >= n ? 'text-indigo-400' : 'text-gray-600'}`}>{label}</span>
              </div>
              {idx < steps.length - 1 && (
                <div className={`w-12 h-px mt-[-16px] transition-colors ${step > n ? 'bg-indigo-500' : 'bg-white/10'}`} />
              )}
            </div>
          ))}
        </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 2 && (
              ...
              >
                ...
                {/* Pack action selector */}
                <div className="space-y-2">
                  {[
                    { key: 'create', emoji: '🏗️', label: 'Create a Pack', desc: 'Start your own group' },
                    { key: 'join',   emoji: '🔗', label: 'Join with code', desc: 'Enter an invite code' },
                    { key: 'skip',   emoji: '⏭️', label: 'Skip for now', desc: 'You can join later' },
                  ].map(({ key, emoji, label, desc }) => (
                    <button
                      key={key}
                      onClick={() => setPackAction(key as typeof packAction)}
                      className={`w-full flex items-center gap-4 p-4 rounded-xl border text-left transition-all ${
                        packAction === key ? 'border-indigo-500/50 bg-indigo-500/10' : 'border-white/8 hover:border-white/20'
                      }`}
                    >
                      <span className="text-2xl">{emoji}</span>
                      <div>
            ...
            {step === 3 && (
              ...
              >
                ...
                <div className="flex gap-3">
                  ...
                  <Button className="flex-1" loading={finishing} onClick={() => finish()}>
                    Let's Go 🔥
                  </Button>
                </div>
              </motion.div>
